...
# ponytail: a fixed small list of personalities, picked once per run, is enough
# to make playthroughs feel different without building a whole trait system.
PLAYER_STYLES = [
    "curious and a bit skeptical, likes asking pointed follow-up questions",
    "friendly and easygoing, tends to agree but still asks genuine questions",
    "blunt and impatient, wants straight answers and calls out vague non-answers",
    "thoughtful and analytical, likes connecting details to the bigger picture",
    "informal and chatty, sometimes goes on a small tangent before circling back",
]
PLAYER_STYLE = random.choice(PLAYER_STYLES)
if AUTO_PLAY:
    console.print(f"[dim]Auto-play: Haiku is playing the human side, style = {PLAYER_STYLE!r}[/dim]\n")

def ai_reply(system, prompt):
    result = subprocess.run(
        ["claude", "-p", "--model", "haiku", "--tools", "", "--system-prompt", system],
        input=prompt, capture_output=True, text=True, timeout=90,
    )
    return result.stdout.strip()

def ai_player_line(character_context, current_convo):
    transcript = "\n".join(f"{'You' if r == 'user' else 'Them'}: {c}" for r, c in current_convo)
    prompt = f"""{character_context}

Your personality this session: {PLAYER_STYLE}.

Conversation so far:
{transcript if transcript else "(nothing said yet — open the conversation)"}

Write your next line as the player."""
    return ai_reply(
        "You write natural, in-character spoken dialogue for the human player in a "
        "historical roleplay game set in pre-1914 Europe. Output only the dialogue line "
        "itself, 1-3 sentences, casual spoken English, no quotation marks, no stage "
        "directions, no meta-commentary, never mention this is a game or that you are an AI. "
        "Stay grounded in the pre-1914 setting — never reference modern institutions "
        "(EU, UN, NGOs), technology, or events; only people, offices, and organizations "
        "that could plausibly exist in that era.",
        prompt,
    )

def run_conversation(messages, character_context=""):
    current_convo = []
    ...
    while True:
        ...
        if AUTO_PLAY:
            if turn > MAX_AUTO_TURNS:
                break
            user_message = ai_player_line(character_context, current_convo)
...
for scenario_key, scenario in convos.items():
    ...
    for situation, topics in zip(situations, topics_sets):
        ...
        run_conversation(messages, system_prompt)
    ...
    scenario_grounding = (
        f"Setting: pre-1914 Europe, in the years before World War One — no modern "
        f"institutions, countries, or figures (no EU, UN, NGOs, etc.), only people and "
        f"organizations that could plausibly exist in that era.\n"
        f"Scenario: {scenario_name}\nBriefing: {scenario['briefing']}\n"
        f"Background discussed so far: {' '.join(situations)}"
    )

    for _ in range(people_needed):
        if AUTO_PLAY:
            who = ai_reply(
                "You name, in a short phrase, who a peace activist in pre-1914 Europe would "
                "want to talk to next given a scenario. Output only the phrase, nothing else, "
                "no explanation, no caveats, no questions back.",
                f"{scenario_grounding}\n\nWho does the player want to talk to next? "
                "Answer in a few words naming a real kind of period-appropriate figure, "
                "e.g. 'a French cabinet minister' or 'the German ambassador in London'.",
            )
            subConvince = ai_reply(
                "You state, in one short sentence, what a peace activist in pre-1914 Europe "
                "wants to convince someone of given a scenario. Output only the sentence, "
                "nothing else, no explanation, no caveats, no questions back.",
                f"{scenario_grounding}\nThe player is about to talk to: {who}\n\n"
                "What does the player want to convince them of or get them to do?",
            )
            console.print(f"[dim]Who do you want to talk to? {who}[/dim]")
            console.print(f"[dim]What do you want to convince them of? {subConvince}[/dim]")
        else:
        ...
        messages = load_roleplay_character(system_prompt)
        current_convo = run_conversation(messages, system_prompt)
        save_conversation(messages)
